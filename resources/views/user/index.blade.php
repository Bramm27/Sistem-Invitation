...
<head>
    ...
    <style>
        ...
        .qr-code {
            ...
            /* Border (optional) */
            padding: 10px;
            /* Padding agar QR code tidak terlalu rapat */
...
<body>
    ...
    <section id="button-penerimaan">
        ...
        @elseif($user->respons == 'hadir')
            <!-- Tampilkan QR Code jika status hadir -->
            <div class="qr-code-wrapper">
                <div class="qr-code" id="qrCodeContainer" style="margin-top: 0rem;">
                    {!! $user->qr_code !!}
                </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <form action="/statusChange" method="POST" enctype="application/x-www-form-urlencoded">
                ...
                <div class="button-container">
                    ...
                    <button type="submit" name="status" value="tidak hadir" class="btn-tidak-hadir">Tidak
                        Hadir</button>
                </div>
            </form>
        </div>
    ...
    <section id="penutup">
        <section class="location" id="map">
            <h1 style="margin-top: 0rem;">Location</h1>
            <iframe
    ...
    <script>
        window.onload = function() {
            ...
            // Jika QR Code ada di server response, simpan ke localStorage
            if (qrCode) {
                localStorage.setItem('qrCode', qrCode.innerHTML);
            }

            // Ambil QR Code dari localStorage jika sudah ada
            var storedQrCode = localStorage.getItem('qrCode');
            if (storedQrCode) {
                qrCode.innerHTML = storedQrCode;
            }
        };
    </script>
</body>
